...
CURSOR_DEFAULTS: dict[str, str] = {
    "scout": "composer-2.5",
    "explore": "composer-2.5",
    "mech-executor": "composer-2.5",
    "executor": "gpt-5.6-terra-medium",
    "verifier": "composer-2.5",
    "security-executor": "inherit",
}
...
def populate_canonical(directory: Path, platform: str, overrides: dict[str, str] | None = None) -> None:
    defaults = CLAUDE_DEFAULTS if platform == "claude" else CURSOR_DEFAULTS
    overrides = overrides or {}
    for role, model in defaults.items():
        write_agent(directory / f"{role}.md", role, overrides.get(role, model))
...
class EscalationPolicyTest(unittest.TestCase):
  ...
  def assert_role_fails(self, platform: str, role: str, model: str) -> None:
    with tempfile.TemporaryDirectory() as tmp:
      ...
      populate_canonical(directory, platform, {role: model})
      failures = validate_directory(directory, platform, self.catalog)
      self.assertTrue(failures, f"expected failure for {platform}/{role} -> {model!r}")
      self.assertTrue(
  ...
  def test_unpinned_extra_agent_fails(self) -> None:
    with tempfile.TemporaryDirectory() as tmp:
      ...
      self.assertTrue(any("custom.md" in line and "no 'model' pin" in line for line in failures))
